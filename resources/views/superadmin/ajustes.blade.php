                ...
                <div class="card-body p-4 pt-2">
                    <form id="form-plan-${plan.id}" onsubmit="event.preventDefault(); guardarPlan(${plan.id});">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control bg-light border-0" id="nombre-${plan.id}" value="${plan.nombre_plan}" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" class="form-control bg-light border-0 fw-bold" id="precio-${plan.id}" value="${plan.precio}" required>
                            </div>
                        </div>
                        <div class="row g-2 mb-3">
                            <div class="col-6">
                                <label class="form-label small text-muted fw-bold text-uppercase" style="letter-spacing: 0.5px;">Máx. Reservas</label>
                                <input type="number" class="form-control bg-light border-0 text-center" id="reservas-${plan.id}" value="${plan.max_reservas_mes}" required>
                            </div>
                            <div class="col-6">
                                <label class="form-label small text-muted fw-bold text-uppercase" style="letter-spacing: 0.5px;">Máx. Admins</label>
                                <input type="number" class="form-control bg-light border-0 text-center" id="usuarios-${plan.id}" value="${plan.max_usuarios_admin}" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label small text-muted fw-bold text-uppercase" style="letter-spacing: 0.5px;">Duración (Días)</label>
                            <input type="number" class="form-control bg-light border-0" id="duracion-${plan.id}" value="${plan.duracion_dias}" required>
                        </div>
                        <div class="mb-4">
                            <label class="form-label small text-muted fw-bold text-uppercase" style="letter-spacing: 0.5px;">Descripción</label>
                            <textarea class="form-control bg-light border-0" id="desc-${plan.id}" rows="2" required>${plan.descripcion || ''}</textarea>
                        </div>
...
async function guardarPlan(id) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            alert('Error al guardar plan: ' + fallbackMsg);
        } else {
            alert(`El plan ${nombre_plan} se actualizó exitosamente.`);
